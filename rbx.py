...
GITHUB_KEY_URL = "https://raw.githubusercontent.com/ChanuX-Srilanka/OldFb/main/key.txt"
YOUR_WHATSAPP_NUMBER = "94740074336"

def get_static_device_id():
    unique_data = (
    ...
    device_hash = hashlib.md5(unique_data.encode()).hexdigest()[:8].upper()
    return f"KEY-{device_hash}"

def check_approval():
    user_key = get_static_device_id().strip().upper()
    
    print("----------------------------------------")
    print(f"🔑 Your Device Key: \033[1;33m{user_key}\033[0m")
    print("----------------------------------------")

    try:
        live_url = f"{GITHUB_KEY_URL}?v={int(time.time())}"
        response = requests.get(live_url, headers={"Cache-Control": "no-cache"})
        
        raw_content = response.text.strip()
        approved_keys = [k.strip().upper() for k in raw_content.splitlines() if k.strip()]
    ...
    if user_key in approved_keys:
        print("\n✅ Access Approved! Welcome Boss...\n")
        
        # English prompt requiring Enter key press
        input("[→] Press Enter after joining...")
        return True
    else:
        print("\n❌ Your Key is NOT Approved yet!")
        print("📲 Opening WhatsApp...")
        
        msg = f"Hello Admin, please approve my Key: {user_key}"
        wa_url = f"https://wa.me/{YOUR_WHATSAPP_NUMBER}?text={requests.utils.quote(msg)}"
        os.system(f"termux-open-url '{wa_url}'" if os.name != 'nt' else f"start {wa_url}")
        sys.exit()
